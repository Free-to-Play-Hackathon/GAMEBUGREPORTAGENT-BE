...
| GeneratingRepro | evidence/context + prompt/schema/model config | validated ReproCase ref |
...
7. Gửi duplicate job cho cùng analysis ID; DB vẫn có một result.
8. Mô phỏng provider timeout; xác nhận bounded retry rồi stable terminal state.
9. Kiểm tra logs/traces nối được API -> outbox -> Worker -> stages mà không lộ raw content.
